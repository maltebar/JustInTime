  ...
  def index
    ...
    if Assignment.exists?(active: true)
      ...
      if !@user.group.writer?
        if @assignment.promoter_due > Time.zone.now && Time.zone.now > @assignment.writer_due
      ...
      if @user.group.writer?
        if @assignment.questions.exists?(user_id: current_user.id, description_flag: "1")
          ...
          if @assignment.description != "" && @assignment.writer_due > Time.zone.now 
        ...
        if @assignment.questions.exists?(user_id: current_user.id, description_flag: "2")
          ...
          if @assignment.description_2 != "" && @assignment.writer_due > Time.zone.now 
        ...
        if @assignment.questions.exists?(user_id: current_user.id, description_flag: "3")
          ...
          if @assignment.description_3 != "" && @assignment.writer_due > Time.zone.now 
